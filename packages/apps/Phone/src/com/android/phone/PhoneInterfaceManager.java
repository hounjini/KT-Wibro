...
/* implementation of iSIMThread for wibro authentication process.
 * by hounjini (hounjini@gmail.com)
 * 										2012. 8. 27
 */

	private static class iSIMThread extends Thread
	{
		private static final int REQUEST_SET_ISIM_AUTH_COMPLETE = 102;
		private static final int REQUEST_RESET_WSIM_STAT_COMPLETE = 103;
		...
		public iSIMThread(IccCard icccard)
		{
			mDone = false;
			mResult = false;
			mSimCard = icccard;
		...
		public void run() {
			Looper.prepare();
			synchronized (iSIMThread.this) {
				mHandler = new Handler() {
					@Override
					public void handleMessage(Message paramMessage) {
						AsyncResult localAsyncResult = (AsyncResult)paramMessage.obj;
						switch (paramMessage.what) {
							case REQUEST_SET_ISIM_AUTH_COMPLETE:
								synchronized (iSIMThread.this) {
									if(localAsyncResult.exception == null)
										mStringResult = (String)localAsyncResult.result;
									mDone = true;
									iSIMThread.this.notifyAll();
									getLooper().quit();
								}
								break;
							case REQUEST_RESET_WSIM_STAT_COMPLETE:
					            synchronized (iSIMThread.this) {
									if(localAsyncResult.exception == null)
										mResult = true;
									mDone = true;
		...
		synchronized String requestSetISIMAUTH(String paramString) {
			while (true) {
				Handler localHandler = mHandler;
				if (localHandler != null)
					break;
			}
			Message localMessage = Message.obtain(mHandler, 102);
			this.mSimCard.requestSetISIMAUTH(paramString, localMessage);
			while (true) {
				if (this.mDone)
					break;
				try { wait(); }
		...
		synchronized boolean requestResetWSIMState() {
			while (true) {
				Handler localHandler = mHandler;
				if (localHandler != null)
					break;
			}
			Message localMessage = Message.obtain(mHandler, 103);
			this.mSimCard.requestResetWSIMState(localMessage);
			while (true) {
				if (this.mDone)
					break;
				try { wait(); }
	...
	/* isimthread for wibro working finish */
...
/* added by hounjini for wibro working */
    public String requestSetISIMAUTH(String s)
    {
        enforceModifyPermission();
        iSIMThread isimthread = new iSIMThread(mPhone.getIccCard());
        isimthread.start();
        try {
	       	Thread.sleep(100);
        } catch (Exception e) {;}
	return isimthread.requestSetISIMAUTH(s);
    }
	...
	return isimthread.requestResetWSIMState();
    }
}
